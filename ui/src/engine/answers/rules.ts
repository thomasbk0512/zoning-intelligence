...
export function getAnswersForZone(zone: string, jurisdictionId: string = 'austin'): ZoningAnswer[] {
  ...
  return intents.map(intent => getAnswerForIntent(zone, intent, jurisdictionId))
}

/**
 * Get answer for a specific intent and zone
 */
export function getAnswerForIntent(zone: string, intent: ZoningIntent, jurisdictionId: string = 'austin'): ZoningAnswer {
